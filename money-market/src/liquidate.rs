...
use super::{borrow, common, errors::*, events, proxies, repay_borrow, seize, storage};

pub type LiquidateBorrowResultType<BigUint> = MultiValue2<EsdtTokenPayment<BigUint>, EsdtTokenPayment<BigUint>>;

#[multiversx_sc::module]
pub trait LiquidateModule: borrow::BorrowModule + common::CommonModule + events::EventsModule + proxies::ProxyModule + repay_borrow::RepayBorrowModule + seize::SeizeModule + storage::StorageModule {
    ...
    fn liquidate_borrow(&self, borrower: ManagedAddress, collateral_market: ManagedAddress, opt_min_tokens: OptionalValue<BigUint>) -> LiquidateBorrowResultType<Self::Api> {
        ...
        self.liquidate_borrow_internal(&liquidator, &borrower, &underlying_amount, &collateral_market, opt_min_tokens)
    }
    ...
    /// - `collateral_market ` - The money market in which to seize collateral from the borrower.
    ///
    fn liquidate_borrow_internal(&self, liquidator: &ManagedAddress, borrower: &ManagedAddress, underlying_amount: &BigUint, collateral_market: &ManagedAddress, opt_min_tokens: OptionalValue<BigUint>) -> LiquidateBorrowResultType<Self::Api> {
        ...
        let liquidation_allowed = self.liquidate_borrow_allowed(&borrow_market, collateral_market, borrower, underlying_amount);
        require!(liquidation_allowed, ERROR_CONTROLLER_REJECTED_LIQUIDATION);

        // repay borrowers debt
        self.repay_borrow_internal(liquidator, borrower, underlying_amount);

        // compute the number of tokens to seize from the borrower's collateral and check
        let tokens_to_seize = self.tokens_to_seize(&borrow_market, collateral_market, underlying_amount);
        ...
        self.liquidate_borrow_event(liquidator, borrower, underlying_amount, collateral_market, &tokens_to_seize);
